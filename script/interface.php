...
global $user, $langs;
	...
	if(!empty($values)){
		...
		$db->query("UPDATE ".MAIN_DB_PREFIX.$task->table_element." SET story_k=".(int)$values['story_k']." WHERE rowid=".$task->id);
	}
...
function _tasks(&$db, $id_project, $status) {
	...
	if($status=='ideas') {
		$sql = "SELECT rowid,story_k FROM ".MAIN_DB_PREFIX."projet_task 
	...
	$TTask = array();
	while($obj = $db->fetch_object($res)) {
		$TTask[] = array_merge( _task($db, $obj->rowid) , array('status'=>$status,'story_k'=>$obj->story_k));
	}
